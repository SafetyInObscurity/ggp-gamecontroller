 ...
 * @param <StateType>
 */
public class HyperStatesTracker<TermType extends TermInterface, StateType extends StateInterface<TermType, ? extends StateType>> {

    protected static final Logger logger = Logger.getLogger(tud.gamecontroller.players.StatesTracker.class.getName());
    ...
    public HyperStatesTracker(GameInterface<TermType, StateType> game, StateType initialState, RoleInterface<TermType> role, int maxStates) {
        this.game = game;
    ...
    public Collection<StateType> statesUpdate(Collection<TermType> seesTerms) {
        Set<StateType> nextPossibleStates = new HashSet<StateType>(maxStates);
        for (StateType state: currentPossibleStates) {
            for (JointMoveInterface<TermType> jointMove: computeJointMoves(state)) {
                if (isPossible(state, jointMove, seesTerms)) {
                    ...
                    if(nextPossibleStates.size() >= maxStates) break;
                }
            }
            if(nextPossibleStates.size() >= maxStates) break;
        }
